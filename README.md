...
![Histograms of exemplary financial ratios and one-year ahead delisting frequency in the data set described in section 2.1. The gray bars represent the total count.](https://github.com/david01neu/delistingml/blob/dev/images/230_histogram_features.svgraw=true)
...
The bottom panels of Figure \ref{fig:random_forest_hyperparameter_metrics} show that different choices of $m$ barely influence the performance metrics on the \ac{OOB} data. Therefore, the default value $m = \frac{\#\text{features}}{3} = 12$ is chosen.
...
*Performance metrics for random forest models with different minimal terminal node sizes $n_{min}$ (top row) and different number of splitting variable candidates $m$ (bottom row). Metrics for in-bag (INB) samples in solid red and for out-of-bag (OOB) samples in dashed blue.*
