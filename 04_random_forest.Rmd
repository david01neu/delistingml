...
if (REDO_MODEL_FITTING) {
  optimization.nodesize <- tibble()
  for (nodesize in seq(5, 100, by = 5)) {
      print(paste("nodesize:", nodesize))
      rf.nodesize.result <- rfsrc(
        response ~ .,
      ...
      train_prediction_table <- tibble(
        ...
        predicted.inb = rf.nodesize.result$predicted,
        predicted.oob = rf.nodesize.result$predicted.oob
      )
      ...
      optimization.nodesize <- rbind(
        optimization.nodesize, append(
        compute.metrics(train_prediction_table$actual == 1,
        train_prediction_table$result.inb),
        list(nodesize = nodesize, samples = 0), after = 0))
      
      optimization.nodesize <- rbind(optimization.nodesize, append(
        compute.metrics(train_prediction_table$actual == 1,
        train_prediction_table$result.oob),
        list(nodesize = nodesize, samples = 1), after = 0))
      
      predicted.test = predict.rfsrc(rf.nodesize.result, newdata = data.test %>% drop_na())
      optimization.nodesize <- rbind(optimization.nodesize, append(
        ...
        list(nodesize = nodesize, samples = 2), after = 0))
    
  }
  colnames(optimization.nodesize) <- c("nodesize", "samples", 
                                            "TP", "FN", "FP", "TN", "accuracy", 
                                            "precision", "recall", "f1", 
                                            "specificity", "mcc")
  # convert binary variable to INB and OOB factors
  optimization.nodesize <- optimization.nodesize %>% 
    mutate(samples = factor(samples,
                            levels = c(0, 1, 2),
                            labels = c("INB", "OOB", "test")))
  save(optimization.nodesize, file = "../data/optimization.nodesize.RData")
} else {
  load("../data_rmrf/optimization.nodesize.RData")
}
...
rmrf <- growRF_Parallel(ntrees = 100,
...
```
```{r}
rf_correct_sample <- growRF_Parallel(ntrees = 100,
                   formula = response ~ capital_ratio + at_turn + inv_turn + rect_turn + pay_turn + cash_ratio + curr_ratio + aftret_eq + aftret_equity + gpm + opmad + roa + roe + de_ratio + debt_assets + pe_exi + response + capital_ratio_h + at_turn_h + inv_turn_h + rect_turn_h + pay_turn_h + cash_ratio_h + curr_ratio_h + aftret_eq_h + aftret_equity_h + gpm_h + opmad_h + roa_h + roe_h + de_ratio_h + debt_assets_h + pe_exi_h + gdpc1_chg + cpiaucsl_chg + fredfunds_chg + sp500_chg | permno,
                   data = data.training %>% select(-public_date),
                   search="ar", # acceptance-rejection trees
                   method="class", # classification
