...
rmrf.pred.probability.test <- predictRF(rmrf, newdata = data.test.2023, prediction = "overall", checkCases = FALSE, corstr = "ar1")

# metrics on test set using best threshold determined based on train data
rmrf.summary.test <- rbind(compute.metrics(data.test.2023$response, rmrf.pred.probability.test >= rmrf.metrics.roc.cutpoint),
      compute.metrics(data.test.2023$response, rmrf.pred.probability.test >= rmrf.metrics$cutpoint.mccf1)) %>%
  cbind(splitting = c("ROC", "MCCF1"), .)
...
rf.pred.probability.test.2023 <- predict.rfsrc(rf, data.test.2023)$predicted
# metrics on test set using best threshold determined based on train data
